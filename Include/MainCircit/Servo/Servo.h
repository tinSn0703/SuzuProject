...
private:
	...
	Uart::ModeTransmit *_mem_uart;
	
	YesNo _mem_is_uart_set :1;
	
public:
	
	Servo();
	Servo(ServoOldNumber _number, Uart::ModeTransmit *_uart);
	Servo(ServoNewNumber _number, Uart::ModeTransmit *_uart);
	
	void Reset(ServoOldNumber _number);
	void Reset(ServoNewNumber _number);
	void Reset(Uart::ModeTransmit *_uart);
...
inline Uart::ModeTransmit * Servo :: Get_uart()
{
	return _mem_uart;
}
