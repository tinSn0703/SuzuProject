	...
	, _controller(Uart::NUM_0)
{
	...
	_wheel.Set_angle_motor_reverse(WheelPlace::FRONT_RIGHT,	YES);
	_wheel.Set_angle_motor_reverse(WheelPlace::BACK_RIGHT,	YES);
	_wheel.Set_angle_motor_reverse(WheelPlace::BACK_LEFT,	YES);
	_wheel.Set_angle_motor_reverse(WheelPlace::FRONT_LEFT,	YES);
	...
	if (_is_unlock)
	{
		switch (_controller.Get_R_stick_y() )
		{
			case Direction::OVER:
			{
				...
				if (_controller.Get_Triangle())	_wheel.Move_wheel_unit(WheelPlace::FRONT_RIGHT, SIGNAL_FORWARD, 8);
				if (_controller.Get_Circle())	_wheel.Move_wheel_unit(WheelPlace::BACK_RIGHT, SIGNAL_FORWARD, 8);
				if (_controller.Get_Cross())	_wheel.Move_wheel_unit(WheelPlace::BACK_LEFT, SIGNAL_FORWARD, 8);
				if (_controller.Get_Square())	_wheel.Move_wheel_unit(WheelPlace::FRONT_LEFT, SIGNAL_FORWARD, 8);
			...
			case Direction::UNDER:
			{
				...
				if (_controller.Get_Triangle())	_wheel.Move_wheel_unit(WheelPlace::FRONT_RIGHT, SIGNAL_REVERSE, 8);
				if (_controller.Get_Circle())	_wheel.Move_wheel_unit(WheelPlace::BACK_RIGHT, SIGNAL_REVERSE, 8);
				if (_controller.Get_Cross())	_wheel.Move_wheel_unit(WheelPlace::BACK_LEFT, SIGNAL_REVERSE, 8);
				if (_controller.Get_Square())	_wheel.Move_wheel_unit(WheelPlace::FRONT_LEFT, SIGNAL_REVERSE, 8);
